...
class MoralisPy:
    ...
    def api_request(self, endpoint, method="GET"):
        ...
        response = requests.request(
            url=self.url_base + endpoint, method=method, headers=headers
        )
    ...
    def convert_native_amount_to_token_amount(
        self, token_address, chain, native_price_amount
    ):
    ...
    def get_token_metadata(self, token_address, chain):
        metadata = self.api_request(
            "/erc20/metadata?chain=" + chain + "&addresses=" + token_address
        )[0]
        return metadata

    def get_total_token_assets(
        self, wallet_address, chains, tokens_to_exclude=None, block=None
    ):
        erc20_token_assets_detail_list = []

        for chain in chains:

            tokens = self.get_tokens_for_wallet(
                wallet_address, chain=chain, block=block
            )
            for token in tokens:
                token_price_object = self.get_token_price(
                    token["token_address"], chain=chain, block=block
                )
                if not token_price_object:
                    pass
                else:
                    token_quantity = float(token["balance"]) * (
                        10 ** -float(token["decimals"])
                    )
                    holdings_value = token_quantity * float(
                        token_price_object["usdPrice"]
                    )
